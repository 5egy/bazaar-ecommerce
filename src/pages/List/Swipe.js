    ...
    <div className="w-full text-center shadow-md shadow-[#6a2d72] py-5">
      ...
      >
      {
        products.map((prod)=>{
          ...
          <SlideItem name={prod.name} rate={prod.rating} price={prod.price} image={prod.mainImage} slug={prod.slug.current}/>
        </SwiperSlide>
        })
      }
      </Swiper>
...
function SlideItem({name, price, image, slug, rate}) {
  ...
  return (
    <div key={name} className="w-3/5 min-w-[200px] md:w-56 lg:w-60 h-full min-h-[250px]  border-2 flex flex-col justify-between border-[#6a2d72] rounded-md">
     <Link  href={`/product/${slug}`}>
      <div className="p-2 text-[#6a2d72]">
        <h3 className="text-sm">{name}
      ...
      <div>
        ...
        <button className="w-full p-2 mt-0 bg-[#6a2d72] text-white">Buy</button>
      </div>
     </Link>
    </div>
  );
